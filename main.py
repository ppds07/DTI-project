...
def Add_Employ():
    ...
    GrossEight = input("Enter Employee Gross Salary for 8Hr: ")
    GrossTwelve = input("Enter Employee Gross Salary for 12Hr: ")
    data = (Id, Name, Email_Id, Phone_no, Address, Post, Salary, GrossEight, GrossTwelve)
    # Instering Employee Details in
    # the Employee (empdata) Table
    sql = 'insert into empdata values(%s,%s,%s,%s,%s,%s,%s,%s,%s)'
    c = con.cursor()
...
def Attendance():
    print("{:>60}".format("-->> Attendance Record <<--\n"))
    Id = input("Enter Employee Id: ")
    # checking If Employee Id is Exit Or Not
    if(check_employee(Id) == False):
        print("Employee Record Not exists\nTry Again")
        ...
        sql = 'insert into attendance values (%s, now())'
        data = (Id,)
        c = con.cursor()
